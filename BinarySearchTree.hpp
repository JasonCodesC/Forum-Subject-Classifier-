...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    ...
    if (height_impl(node->left) < height_impl(node->right)) {
      max = height_impl(node->right);
    }
    else {
      max = height_impl(node->left);
    }
    return 1 + max;
  }
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    Node* rut = new Node;
    rut->datum = node->datum;
    rut->left = copy_nodes_impl(node->left);
    rut->right = copy_nodes_impl(node->right);
    return rut;
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if (node->left && (!less(max_element_impl(node->left)->datum, node->datum))) {
      return false;
    }
    if (node->right && less(max_element_impl(node->right)->datum, node->datum)) {
      return false;
    }
    if (!node->left && node->right) {
      bool val;
      val = less(node->datum, node->right->datum) &&
      check_sorting_invariant_impl(node->right, less);  
      return val;
    }
    if (node->left && !node->right) {
      bool val;
      val = less(node->left->datum, node->datum) && 
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less){
    ...
    (!less(node->datum, val) && !less(val, node->datum))) {
      ...
      Node *rut = min_greater_than_impl(node->left, val, less);
      if (!rut) {
        return node;
      }
      else {
